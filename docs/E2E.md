...
|------|-----------|----|--------------|-----|-----|-----|-------|
| 2026-07-11 | 72885c5 (relay 45fda60) | ✅ text | ✅ ~935 | ✅ hold | ⏳ | ⏳ | adb-driven pass. §8: zero-tap text write `confirmed` (D3), screen off, quiet receipt — image/settings-off/MIUI-blocked variants not re-run. §9: 10-sample median e2eMs 855ms + sinceDetectMs ~80ms ≈ 935ms (≪2000); burst 6→1 supersede on-device; 7-event instrumentation all present; camera/browser filter (§9.5) not run. §10.1–2: `screenshot_held` offline, then clean republish on reconnect (device_connected→auth_ok→payload_published(image)→clipboard_write, no stale drop) — **required fixing a relay startup-republish bug that dropped held frames (commit 45fda60)**; §10.3 doze (30min) not run. §11 (needs 5min backoff-at-cap) and §12 (60min idle) time-gated, pending. Also fixed this session: wl-copy ack stall (17s→17ms) and jpeg→png paste refusal. |
