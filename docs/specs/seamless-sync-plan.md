...
Resolves [#32 Assemble the seamless-sync implementation plan](https://github.com/Snehit70/vidyut/issues/32),
the final ticket of the [Seamless Sync map](https://github.com/Snehit70/vidyut/issues/23).
...
| Spec | Ticket | Owns |
| --- | --- | --- |
| [`relay-observability.md`](relay-observability.md) | [#25](https://github.com/Snehit70/vidyut/issues/25) | Relay/laptop JSON log events, correlation keys, bar measurement from relay stdout |
| [`zero-tap-receive.md`](zero-tap-receive.md) | [#29](https://github.com/Snehit70/vidyut/issues/29) | Clipboard plugin, silent service-side write, failure taxonomy, quiet receipts |
| [`keepalive-reconnect.md`](keepalive-reconnect.md) | [#24](https://github.com/Snehit70/vidyut/issues/24) | WS pings, connect timeout, backoff retune, screen-on reconnect trigger |
| [`screenshot-observer.md`](screenshot-observer.md) | [#27](https://github.com/Snehit70/vidyut/issues/27) | Observer plugin, MIUI-safe filter, dedup/watermark, permission grant states |
| [`screenshot-auto-push.md`](screenshot-auto-push.md) | [#28](https://github.com/Snehit70/vidyut/issues/28) | Push controller, latest-wins slot, offline hold + ack, crypto cost fixes, toggle |
| [`onboarding-permissions.md`](onboarding-permissions.md) | [#30](https://github.com/Snehit70/vidyut/issues/30) | First-run wizard, setup checklist, MIUI guide, settings additions |
| [`read-logs-auto-text.md`](read-logs-auto-text.md) | [#31](https://github.com/Snehit70/vidyut/issues/31) | Opt-in READ_LOGS auto-text mode, echo guard, advanced screen |
| Notification-surface decisions | [#33](https://github.com/Snehit70/vidyut/issues/33) | What survives after zero-tap (resolution comment on the ticket — no spec file) |
...
[#26](https://github.com/Snehit70/vidyut/issues/26).
...
1. **App-local Flutter plugins for anything the service isolate must reach.**
   ...
   - `app/packages/vidyut_clipboard/` — clipboard writes (WP2), plus the
...
- New `app/packages/vidyut_clipboard/` — Kotlin `FlutterPlugin` registering the
  `vidyut/clipboard` MethodChannel from **application context**; `writeText`,
  `writeImage(path, mime)` (moved verbatim from `MainActivity`).
- `app/android/app/src/main/kotlin/dev/snehit/vidyut/vidyut/MainActivity.kt` —
  drop the `vidyut/clipboard` registration (multicast channel stays).
...
- `LocalPayloadNotifier` (in `payload_receiver.dart`) — new `vidyut_receipts` channel,
  `Importance.low`; delete the old `vidyut_payloads` channel on upgrade; fixed
...
- `app/packages/vidyut_clipboard/` — `ACTION_SCREEN_ON` `BroadcastReceiver` registered
...
- New `app/packages/screenshot_observer/` — `ScreenshotObserverPlugin.kt` + singleton
  ...
  Channels: `vidyut/screenshot_observer` (`start`/`stop`/`accessLevel`) and
  `vidyut/screenshot_events` (event JSON per §3).
...
- `app/lib/src/foreground/vidyut_foreground_service.dart` — construct/inject the
...
- `app/packages/vidyut_clipboard/` — manufacturer method (D4); MIUI deep-link intents
...
**Decisions:** [#33 resolution comment](https://github.com/Snehit70/vidyut/issues/33).
...
- `app/lib/src/foreground/vidyut_foreground_service.dart` — status text stays
...
- New Advanced → Clipboard auto-send screen off `settings_screen.dart` — toggle, live
  grant state, copy-paste adb block for `dev.snehit.vidyut.vidyut`, MIUI caveat line.
...
- **Phone → laptop screen-on ≤2s:** per screenshot,
  ...
  `journalctl --user -u vidyut-relay -o cat | jq -r 'select(.message=="clipboard_write") | .e2eMs'`
...
- **Idle survival:** ≥60 min screen-off with MIUI toggles applied (lock-in-recents +
  ...
  `journalctl --user -u vidyut-relay -o cat | jq -r 'select(.message=="socket_reaped")'` → empty.
...
- **Out of scope (map-locked):** Play Store/signed releases, non-LAN transport,
  ...
  (that's the [v1 map](https://github.com/Snehit70/vidyut/issues/9)'s #22).
